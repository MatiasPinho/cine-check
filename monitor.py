...
def load_state(path: Path) -> list[dict[str, str]] | None:
    if not path.exists():
        return None
...
def save_state(path: Path, showings: list[dict[str, str]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    content = json.dumps(
        {"version": 1, "showings": showings},
        ensure_ascii=False,
        indent=2,
    ) + "\n"
...
def run(state_path: Path) -> None:
    ...
    if previous is None:
        ...
        previous_keys = {showing_key(showing) for showing in previous}
        new_showings = [showing for showing in current if showing_key(showing) not in previous_keys]
        if new_showings:
